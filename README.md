# Hi, I’m Flavio Drogo 👋
Civil Engineering student with a focus on Computer Science and Management, specializing in Artificial Intelligence and Decision Support at the Faculty of Engineering, University of Mons.

---

## 📂 Projects & Experience
### Academic Projects
- **Drone Project UMONS** – Led the construction of a drone, managing a small team and applying engineering and programming skills.
- **Brussels Waste Optimization** – Optimized garbage truck routes using methods learned in optimization courses, working with a real client.
- **Research Initiation** – Assisted a PhD student on a specific research aspect, developing skills in data analysis and scientific methodology.

### Competitions
- **BAPC & NWERC** – Solved algorithmic problems under time constraints in team competitions, enhancing teamwork, problem-solving, and algorithmic skills.

### Personal / AI Projects
- **Reinforcement Learning Pacman** - Simple DQN for Pacman Atari version.
---

## 💻 Skills
- **Programming:** Python, C++, Java, SQL
- **AI & Data Analysis:** Machine Learning, Decision Support, Optimization
- **Other:** Project Management, Teamwork, Communication

---

## 📫 Contact
- Email: [drogoflavio16@gmail.com](mailto:drogoflavio16@gmail.com)
- LinkedIn: [Drogo Flavio](https://www.linkedin.com/in/drogo-flavio-6309aa252/)
